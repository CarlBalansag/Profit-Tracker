...
function reset() {
  ...
  db={user:[{id:ids.user,username:'QA Fixture',tutorial_seen:true,calendar_token:'qa-only-token',accounting_preferences:null}],
    ...
    expense:[{id:randomUUID(),user_id:ids.user,name:'QA Storage',amount:25,date:new Date('2026-09-01T12:00:00Z'),category:'Storage'}],recurringExpense:[],account:[],goal:[],calendarEvent:[],productNote:[],authAttemptBucket:[]};
}
reset();
...
for(const model of Object.keys(db)) {
  ...
  for(const op of ['findMany','findUnique','findFirst','create','createMany','update','updateMany','delete','deleteMany','upsert'])prisma[model][op]=async(args={})=>{
    ...
    if(op==='create'||op==='createMany'||(op==='upsert'&&!rows.length)) {
      ...
      const created=(Array.isArray(data)?data:[data]).map(x=>({id:randomUUID(),...(model==='inventory'?{status:'PURCHASED',created_at:new Date()}:{}),...(model==='authAttemptBucket'?{count:1}:{}),...x}));
