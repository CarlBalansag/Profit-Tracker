...
## Schedule C: first expense worksheet

Implemented the agreed first focused feature: per-user Settings opt-in, Expenses business/mixed/personal classification, payee/purpose/payment evidence, explicit payment status/date, reviewed tax category, supporting records/review editor, annual Decimal totals, combined Part V total, CSV/JSON export and browser print/save-PDF action. Existing expense amounts and dashboard preferences remain intact. Missing inputs remain saveable and cannot silently enter reviewed totals. Financial/classification edits reset review; version checks reject stale reviews or concurrent expense saves.

- Affected routes: expenses PUT/POST, preferences schedule-c GET/PUT, new schedule-c GET. Models: additive User.schedule_c_enabled, Expense.tax_details JSONB and Expense.tax_version. No hosted migration or live data writes performed.
- Nine focused HTTP regressions passed: typed preference/ownership/failure/retry, payment-year boundaries, owner isolation, read-only reporting, precise per-expense mixed-use rounding, combined Other expenses, personal/unpaid/unreviewed exclusions, unknown dates, invalid metadata/calendar dates/review claims, partial omission, failed-save preservation/retry/repeat/delete, empty totals and stale-version rejection.
- Seven focused UI cases passed: opt-in, future-year labels/filters/export records, failed draft/retry/cancel, required review details versus incomplete drafts, pending duplicate/dismissal guards, preference failures and ordinary expense entry. Full local suites: API 155 passed with ten native cases skipped without QA_DATABASE_URL; frontend 81 passed. Zero-warning lint, Prisma validation/client generation and production/PWA build passed; existing bundle warning remains.
- Migration chain applies all 16 checked-in migrations to a fresh disposable database, including JSONB/boolean columns. Two new native PostgreSQL cases cover metadata persistence, ownership, precise business adjustments, partial writes/stale review and overlapping amount/review writes; the existing CI workflow enables all ten native cases against disposable PostgreSQL 16. Check the feature commit's workflow result for native execution evidence.
- Browser fixture: opt-in, review/save, $25 expense at 80% -> $20 worksheet while Expenses stays $25, incomplete $50 mailer expense -> pending/$0 worksheet, future 2030 selection excludes the 2026 payment, cancellation and 390x844 mobile scrollable dialog. Document width matched viewport at sampled widths. Fixtures only; no user records altered.
- Native execution verified: [CI run 34730230176](https://github.com/CarlBalansag/Profit-Tracker/actions/runs/34730230176) on feature commit 8c64d29 passed all gates, including disposable PostgreSQL migration deployment and all ten native API cases (two new Schedule C cases), frontend tests, zero-warning lint and build.
- Limits: paid cash-method expense preparation only; mapping year 2025, all other years explicitly planning. Income, inventory/COGS, accrual, deferred deductions, expense splits/refunds and year-end locking are future focused stages. Receipt links/evidence references support review; receipts are not automatically validated. Browser PDF/OS-save completion, real receipt transfers and production deployment were not exercised. UTC expense-date default was recorded in BACKLOG.md as a separate task.
